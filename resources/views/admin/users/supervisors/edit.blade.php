    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title"> تعديل المشرف </h6>
                <button aria-label="Close" class="close" data-dismiss="modal"
                        type="button"><span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form method="post" action="{{url('admin/supervisor/update')}}" autocomplete="off">
                @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="regions" id="regions_{{$user['id']}}" class="form-control">
                            ...
                            @foreach($regions as $region)
                                <option {{$user['regions'] == $region['id'] ? 'selected':''}} value="{{ $region['id'] }}">{{ $region['name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
